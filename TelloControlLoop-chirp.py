...
def send(message):
  ...
  try:
    ...
    print("time: %10.4f  Sending message: %s" % (current_time-prev_time, message))
...
def receive():
  ...
  while True:
    ...
    try:
      ...
      current_time = time.time()
      print("time: %10.4f  Received message: %s" % (current_time-prev_time, response.decode(encoding='utf-8')))
      prev_time = current_time
    except Exception as e:
...
print('Type in a Tello SDK command and press the enter key. Scripted commands will execute. Enter "quit" to exit this program.')
...
while True:
  ...
  try:
    ...
    send(message)
    sleep(10) # wait for takeoff and motors to spin up
    message = 'battery?'  # give easy command to see if takeoff has finsihed
    send(message)
    sleep(0.5)
    message = 'speed 50'
    send(message)
